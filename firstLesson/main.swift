...
/*
 ...
 Any -- Это общий тип данных который может содержать все частные типы. Когда у нас переменная или константа указан с типом Any, то компилятор не видит значение что указано после знака "=", поэтому надо аккуратно. Для компилятора существует только сам тип Any, через print() покажет то что есть, но никакие математические или какие либо операции с ним невозможно производить.
 ...
 Оператор "is" используется для проверки содержаемого Any на соответствие ожидаемых типов. Пример:
    ...
    Этот оператор похож на оператор в PYTHON if isinstance
*/
